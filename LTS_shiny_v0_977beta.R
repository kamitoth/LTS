...
server <- function(input, output, session) {
  ...
  observe({
    ...
    non_na_repl <- sum(colSums(!is.na(df[,-(1L:2L)])) > 0)
    ...
    # Convert df to long format
      ...
      # Model selection 
      # The most complex model based on the provided data
      #BUG: ERROR if no random effect data provided
      uc <- try(model.sel(dat=df_long, unique_t=unique_time, unique_r=unique_run, unique_u=unique_unit_no), F)
      
      # Removing 'anal' if it does not improve the model
      # Rule no1: only weak 'anal' correlations (<0.3) are allowed with both 'time' and 'unit_no'
      form <- formula(uc)
      print(form)
      uc2 <- lmer(update_formula(formula = form, term_to_remove = "anal"), df_long)
      print(uc2)
      print(uc)
      print(str(df_long))
      
      if(!is.null(df$anal)){
        corat <- try(cor(df_long$anal, df_long$time), F)
        corau <- try(cor(df_long$anal, df_long$unit_no), F)
        if(!is.null(corat) | !is.null(corau)) {
          if(corat > 0.3 | corau > 0.3) {
            uc <- uc2
          }
        }
        print(cor(df_long$anal, df_long$time))
      }
      
      #uc <- update(uc, . ~ ., data=uc2$model)
      #uc2 <- update(uc2, . ~ ., na.action = na.exclude)
        
      model_comp <- try(anova(uc, uc2), F)
      print("modelcomp")
      print(class(uc))
      print(class(uc2))
      print(model_comp)
