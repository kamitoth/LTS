...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # Convert df to long format
      ...
      df_long <- df_long[colSums(!is.na(df_long)) > 0] # foul-proofing: delete columns with all NA values if exist
      ...
      # THE ENGINE
      #req(all(rowSums(!is.na(df[,-(1L:2L)])) > 1)) #min. 2 replicates per time point is required #### MAYBE if is needed!
      print("rowsums")
      ...
      # if more then 1 units were tested
        ...
        if (all(colSums(M > 0L) == 1L)) {
          ...
          # If significant, add analytical sequence to the model
          if (sig_slope < 0.05){
            uc <- lmer(res ~ time + anal + (1|run/unit_no), df_long)
            # Otherwise use simpler model
          } else {
            uc <- lmer(res ~ time + (1|run/unit_no), df_long)
          }
          ...
          sig_slope2 <- anova(lmerTest::lmer(res ~ time + anal + (1|run) + (1|unit_no), df_long))['anal','Pr(>F)'] # to obtain approximate p-values
          
          # With 'anal' in the model if analytical trend is significant...
          if (sig_slope2 < 0.05) {
            uc <- lmer(res ~ time + anal + (1|run) + (1|unit_no), df_long)
